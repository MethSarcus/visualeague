      ...
      <Modal size={"sm"} isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent bg={"#1A202E"} color={"white"} overflowX={"hidden"}>
          ...
          <ModalBody>
            ...
            {(context as League).settings.roster_positions?.filter(rosPos => rosPos == "BN").map((pos, index) => {
            return <PositionalMatchupContainer key={index} position={pos as LINEUP_POSITION} homePlayer={props.matchup?.homeTeam.bench.at(index) ?? new MatchupPlayer()} awayPlayer={props.matchup?.awayTeam?.bench.at(index) ?? new MatchupPlayer()}/>
            }) as any}
            </Box>
          </ModalBody>
